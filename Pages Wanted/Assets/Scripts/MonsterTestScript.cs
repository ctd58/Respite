...
public class MonsterTestScript : MonoBehaviour {
    ...
    /// The idea of this script is to start moving forward to the waypoint system
    /// and less from the speed system. This requires much testing. As such, it would be 
    /// appreciated if this script was not deleted or changed without confirmation
    /// from Widchard Faustin or Lily Lauben. 
    /// </summary>
    /// 

    //This list of variables may need to be slimmed. 
    ...
    [Range(0.0F, 5.5F)] public float chaseTime = 5.0f;
    ...
    void checkprefs()
    {
        ...
        if (PlayerPrefs.GetFloat("monstersense") == 0.0f || PlayerPrefs.GetFloat("monstersense") < 2.0f || PlayerPrefs.GetFloat("monstersense") > 10.0f)
        {
    ...
    public void CreateNewWaypoint()
    {
        ...
        if (target != null)
        {
            wayPoints.Insert(index + 1, target);
        }
    }

    public void CreateNewWaypoint(Transform x)
    {
        wayPoints.Insert(index + 1, target);
    }
